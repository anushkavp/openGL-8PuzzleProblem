...
int count=0;
char countstr[10];
int cx[10] = { 500,360,740,1110,360,740,1110,360,740,1110 };
int cy[10] = { 500,1145,1145,1145,770,770,770,380,380,380 };
...
void final()
{
	...
	glClear(GL_COLOR_BUFFER_BIT);
	glColor3f(1.0,1.0,0.0);
	drawString(400,1000,"CONGRATULATIONS! YOU WON!!");
	
	sprintf(countstr,"number of steps= %d",count);
	drawString(400,800,countstr);
	glColor3f(1.0,1.0,1.0);
	glFlush();
	//getchar();
...
void mouse(int btn, int state, int x, int y)
{
	...
	if(btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		for(int i=1;i<=9;i++)
		{		
			if(mx<(cx[i]+160) && mx>(cx[i]-160) && my<(cy[i]+160) && my>(cy[i]-160)) 
			{
				//printf("i=%d",i);
				if(arr[i-1]==0 && (i-1)>0 && (i-1)<=9 && (i==2|| i==3 ||i==5||i==6||i==8||i==9)) swap(i,i-1);
				if(arr[i+1]==0 && (i+1)>0 && (i+1)<=9 && (i==2|| i==1 ||i==5||i==4||i==8||i==7)) swap(i,i+1);
				if(arr[i-3]==0 && (i-3)>0 && (i-3)<=9) swap(i,i-3);
				if(arr[i+3]==0 && (i+3)>0 && (i+3)<=9) swap(i,i+3);
				//for(int j=1;j<=9;j++) printf("%d",arr[j]);
				//printf("\n");
			}
		}
	}
...
{                                                                                                                                                              
	...
	glutInitWindowSize(1200,600);//800,600
